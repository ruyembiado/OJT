            ...
            <div class="card bg-dark shadow border-0 card-radius">
                ...
                <div class="card-footer border-0 d-flex align-items-center justify-content-between">
                    ...
                    <?php } else if ($result['status'] == 1) {
                        $responded = firstJoin('users', [['emergency_result', 'emergency_result.rescuer_id', 'users.id']], ['emergency_result.emergency_id' => $result['emergency_id']]) ?>
                        <p class="m-0 text-secondary">Rescuing by: <span class="text-light"><?php echo !empty($responded) ? $responded['name'] : '' ?></span></p>
                    <?php } ?>
